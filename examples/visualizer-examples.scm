...
;;; Visualizer examples - generates Mermaid diagrams for interesting functions

(use-modules (ice-9 format)
             (srfi srfi-1))

;; Simple Mermaid diagram generator (without the broken module)
(define (generate-mermaid-cps-flow name fn-calls)
  (string-append
   "graph TD\n"
   ...
   (string-join fn-calls "\n")
   "\n"))

;; Example 1: Fibonacci with CPS
(define (fib-cps-diagram)
  (display "=== Fibonacci CPS Flow ===\n")
  (display (generate-mermaid-cps-flow
            "fibonacci-cps"
...
;; Example 2: GCD (Greatest Common Divisor) with CPS
(define (gcd-cps-diagram)
  (display "\n=== GCD CPS Flow ===\n")
  (display (generate-mermaid-cps-flow
            "gcd-cps"
...
;; Example 3: Product of list with CPS
(define (product-cps-diagram)
...
;; Example 4: Ackermann function with CPS (simplified)
(define (ackermann-cps-diagram)
...
;; Example 5: Tree traversal with CPS
(define (tree-sum-cps-diagram)
  (display "\n=== Tree Sum CPS Flow ===\n")
  (display (generate-mermaid-cps-flow
            "tree-sum-cps"
...
(display "=== Generating Mermaid Diagrams for CPS Functions ===\n\n")

(fib-cps-diagram)
(gcd-cps-diagram)
(product-cps-diagram)
(ackermann-cps-diagram)
(tree-sum-cps-diagram)

(display "\n=== Diagrams Generated Successfully ===\n")
(display "Copy any diagram above into a .mmd file or use in org-mode with:\n")
(display "#+begin_src mermaid\n")
(display "[paste diagram here]\n")
(display "#+end_src\n")